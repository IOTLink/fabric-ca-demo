...
var allFabricCAConfig Config

type CA struct{
	CaService *FabricCA //*fabricCAClient.FabricCA
...
func (c *CA)InitCaServer(orgId string, enroll_user_dir string) error {
	...
	if err != nil {
		log.Println("MspID() returned error: ", err)
		return err
	...
	c.Client = NewClient(allFabricCAConfig)

	err = bccspFactory.InitFactories(allFabricCAConfig.CSPConfig("crypto1"))
	if err != nil {
	...
	c.CaService, err = NewFabricCAClient(allFabricCAConfig, orgId)
	if err != nil {
