...
func NewFabricCAClient(config Config, org string) (*FabricCA, error) {
	if org == ""  {
	...
	c.Config.CSP = config.CSPConfig("crypto1")
...
func (fabricCAServices *FabricCA) createSigningIdentity(user sdkApi.User) (*fabric_ca.Identity, error) {
	// Validate user
	if user == nil {
